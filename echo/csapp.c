...
int open_clientfd(char *hostname, char *port) {
    ...
    hints 준비 (클라이언트 측 필터 조건)*/
    memset(&hints, 0, sizeof(struct addrinfo));
    hints.ai_socktype = SOCK_STREAM;  /* Open a connection TCP 연결 지향 소켓 */
    hints.ai_flags = AI_NUMERICSERV;  /* 숫자 포트 번호 명시 */
    hints.ai_flags |= AI_ADDRCONFIG;  /* 네트워크 능력(IPv4/IPv6)에 맞춰 getaddrinfo 결과를 현실적으로 줄여주는 최적화 플래그 */
    
    /* 성공 시 listp는 IPv4/IPv6 후보 주소들의 연결 리스트
       실패 시 -2 반환 (에러 메세지) */
